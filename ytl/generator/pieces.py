...
def generate_random_piece(**kwargs):
	...
	length_probs = kwargs.get('length_probs', np.ones(len(length_options))/len(length_options))
	width_probs = kwargs.get('width_probs', np.ones(len(width_options))/len(width_options))
	height_probs = kwargs.get('height_probs', np.ones(len(height_options))/len(height_options))
	weight_probs = kwargs.get('weight_probs', np.ones(len(weight_options))/len(weight_options))
	stack_probs = kwargs.get('stack_probs', np.ones(len(stack_options))/len(stack_options))
	num_pieces_probs = kwargs.get('num_pieces_probs', np.ones(len(num_pieces_options))/len(num_pieces_options))
	piece_type_probs = kwargs.get('piece_type_probs', np.ones(len(piece_type_options))/len(piece_type_options))
	return {
